...
class CompanyPermitRequestList extends Component {
    ...
    requestPermit = (e) => {
        this.setState({ isRequestPermit: true, mode: 'request' });
    }
    bindPermits() {
    ...
    deletePermit(permitid) {
        ...
        if (!window.confirm("Do you want to delete permit with Id: " + permitid))
            ...
            this.setState({ loading: true });
            fetch(config.webApiUrl() + 'aptc_permit/' + permitid, { method: 'delete' })
                .then(res => {
                    this.bindPermits();
            }).catch((error) => {
            ...
            contents = (
                <div>
                    ...
                    {this.state.isRequestPermit === true &&
                        <RequestPermit
                            companyid={this.props.location.state.vec.companyid}
                                                                                    ...
                                                                                    <li className="dropdown-spacing table-dropdown-small-width">
                                                                                        <button
                                                                                            className={!this.state.show
                                                                                                ? ""
                                                                                                : this.state.show}
                                                                                            onClick={(e) => this.deletePermit(vec.permitid)}>Delete</button>
                                                                                    </li>
                                                                                </ul>
                                                                            ) :
                                                                                (<ul className="dropdown-menu dropdown-menu3 dropdown-inverse pull-right" aria-labelledby="dropdownMenu4">
                                                            ...
                                                            <div className="">
                                                                <div className={localStorage.getItem('selectedLanguageCode') == 1
                                                                    ...
                                                                    <p> <span className={localStorage.getItem('selectedLanguageCode') == 1
                                                                        ...
                                                                        <span className="btn-group" >
                                                                            <i className="dropdown-arrow dropdown-arrow-inverse" />
                                                                            <button dir="ltr" className={this.state.vec.statusid == (420).toString() ? "btn btn-active status dropdown-toggle status-title" : ([this.state.vec.statusid == (422).toString() ? "btn btn-status-rejected dropdown-toggle status status-title-rejected" : "btn btn-status-approved dropdown-toggle status status-title-approved"])} data-toggle="dropdown">
