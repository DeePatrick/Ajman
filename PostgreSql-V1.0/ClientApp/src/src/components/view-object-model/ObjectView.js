...
class ObjectView extends Component {
    ...
    componentWillReceiveProps(nextProps) {
        ...
        switch (nextProps.Action) {
            ...
            case "Fines":
                if (nextProps.ActionType === "EMP") {
                    url = " aptc_individual_getAllActiveVehicles/" + localStorage.getItem('selectedLanguageCode') + "/" + nextProps.individualid;
                }
                else {
    ...
    render() {
        ...
        if (this.state.Action === "Ownership" && this.state.datalist.length > 0) {
            if (this.state.datalist.length > 0) {
                filteredRoles = this
                    ...
                    .datalist
                    .filter((vec) => {
                        return (vec.ownernameen.toLowerCase().indexOf(this.state.search.toLowerCase()) !== -1
                        );
                    });
            }
        }
        ...
        if (this.state.Action === "Permits" && this.state.datalist.length > 0) {
            filteredPermits = this.state.datalist.filter((per) => {
                if (per.docTypeName === null || per.indivName === null) {
                    return (
                        "No Permit"
                    );
                }
                    ...
                    return (
                        per.docTypeName.toLowerCase().indexOf(this.state.search.toLowerCase()) !== -1 ||
                        per.indivName.toLowerCase().indexOf(this.state.search.toLowerCase()) !== -1
                    );
                }
                                                    ...
                                                    {filteredRoles.map((owner, index) => (
                                                        <ul className="list-group" key={index}>
                                                            <li className="list-group-item list-group-item-action">  <img
                                                                ...
                                                                alt="woman"
                                                                height="40"
                                                                width="40" /> &nbsp;{owner.ownernameen} &nbsp;{owner.ownernamear} &nbsp; {owner.ownerroletype} &nbsp; &nbsp;{owner.nationality} &nbsp;{owner.ownerrole}
                                                            </li>
                                                        </ul>
                                                    ))}
                                                </div>
                                                    ...
                                                    {filteredRoles.map((owner, index) => (
                                                        <ul className="list-group" key={index}>
                                                            <li className="list-group-item list-group-item-action">  <img
                                                                ...
                                                                alt="woman"
                                                                height="40"
                                                                width="40" /> &nbsp;{owner.ownerNameEn} &nbsp;{owner.ownerNameAr} &nbsp; {owner.ownerRoleType} &nbsp; &nbsp;{owner.nationality} &nbsp;{owner.ownerRoleID}
                                                            </li>
                                                        </ul>
                                                    ))}
                                                </div>
                                                        ...
                                                        <ul className="list-group" key={index}>
                                                            <li className="list-group-item list-group-item-action">
                                                                ...
                                                                <span className="veh-text" style={{ marginLeft: '10px' }}>
                                                                    {per.documenttypename}&nbsp;&nbsp;{per.documentnumber} &nbsp;{per.validto} to {per.validfrom}
                                                    ...
                                                    {filteredIndividuals.map((individual, index) => (
                                                        <ul className="list-group" key={index}>
                                                            <li className="list-group-item list-group-item-action">
                                                                <img
                                                                    id="icon-pics"
                                                                    src={!individual.profilephoto
                                                                        ? require('../../assets/user-img.png')
                                                                        : individual.profilehoto}
                                                                    onError={(e) => {
