...
export class EditIndividual extends Component {
    ...
    bindEmployee(employeeid) {
        fetch(config.webApiUrl() + "aptc_employee_edit/" + employeeid)
            .then(response => response.json())
            .then(data => {
    ...
    fileSelectedHandler(e) {
        if (e.target.files && e.target.files[0]) {
            ...
            reader.onload = (e) => {
                this.setState({ profilephoto: e.target.result, filename: file.name });
            };
    ...
    handleSave() {
        this.props.saveModalDetails(this.state.employeeid, 'edit');
    }
    onSubmit(e) {
        e.preventDefault();
        this.setState({ loading: true });
        const employee = JSON.stringify(this.state);
        axios
            .put(config.webApiUrl() + 'aptc_employee ', employee, {
                headers: {
                    'Content-Type': 'application/json'
                }
            })
    ...
    render() {
        return (
            <div className="modal" id="editModal" tabIndex="-1" role="dialog" aria-labelledby="editModalLabel">
...
export default translate(EditIndividual);
