...
export class RequestPermit extends Component {
    ...
    componentWillReceiveProps(nextProps) {
        ...
        this.initializeData();
        if (nextProps.mode === 'request') {
            this.setState({ isPayment: false, companyid: nextProps.companyid });
            ...
            fetch(config.webApiUrl() + "/aptc_getCommonMasters/" + localStorage.getItem('selectedLanguageCode') + "/23")
                ...
                .then(data => {
                    ...
                    this.bindVehicle(nextProps.companyid);
                });
    ...
    bindEmployee() {
        fetch(config.webApiUrl() + "aptc_getAllActiveDrivers/1/15")
            .then(response => response.json())
            .then(data => {
    ...
    bindVehicle(companyid) {
        fetch(config.webApiUrl() + "aptc_company_vehicles/" + localStorage.getItem('selectedLanguageCode') + "/" + companyid)
            .then(response => response.json())
            .then(data => {
    ...
    bindEmployeebyrole(roleid) {
        fetch(config.webApiUrl() + "aptc_company_getAllActiveDrivers/" + localStorage.getItem('selectedLanguageCode') + "/" + this.state.companyid + "/" + roleid)
            .then(response => response.json())
            .then(data => {
    ...
    onChangePermitType = (e) => {
        ...
        this.setState({
            ...
            carplatepermitlist: [],
            createdby: localStorage.getItem('individualid'),
            individualid: localStorage.getItem('individualid')
        });
        if (e.target.selectedIndex > 0) {
            ...
            switch (this.state.permittypelist[e.target.selectedIndex - 1].code.toUpperCase()) {
                ...
                case "FO-FR09":
                    ...
                    break;
                case "FO-FR10":
                    this.bindEmployeebyrole(15);
                    this.bindDuration(e.target.value, 0);
                    this.setState({ driverlabel: 'Driver (Company)', isDriver: true, isDuration: true });
                    break;
                case "FO-FR11":
                    this.bindEmployeebyrole(17);
                    this.bindDuration(e.target.value, 0);
                    this.setState({ driverlabel: 'Conductor (Company)', isDriver: true, isConductor: true, isDuration: true });
                    break;
                case "FO-FR12":
                    this.bindEmployeebyrole(16);
                    this.bindDuration(e.target.value, 0);
                    this.setState({ driverlabel: 'Representative (Company)', isDriver: true, isCompRep: true, isDuration: true });
                    break;
                case "FO-FR13":
                    this.bindWeight();
                    this.setState({ isPrivate: true, isWeight: true });
                ...
                case "FO-FR14":
                    this.bindCompany();
                    this.bindDuration(e.target.value, 0);
                    this.setState({ isGPS: true, isComapny: false });
                    break;
    ...
    driverChange = (e) => {
        ...
        for (var i = 0; i < this.state.driverlist.length; i++) {
            if (e.target.value === this.state.driverlist[i].individualid.toString()) {
                    this.setState({ companyid: this.state.driverlist[i].companyid });
                    break;
                }
            }
    };
    ...
    savePermit(e) {
        ...
        permit.individualid = this.state.individualid;
        permit.companyid = this.state.companyid;
        permit.driverid = this.state.driverid;
        ...
        if (this.state.isthisreligionsschool === true) {
            permit.amount = 0;
            permit.totalpermitfee = 0;
        }
        parmitlist.push(permit);
        permitDetails.createdby = localStorage.getItem('userid');
        if (this.state.isthisreligionsschool === true) {
            permitDetails.totalpermitfee = 0;
        }
        else { permitDetails.totalpermitfee = this.state.price; }
                        ...
                        <div className="modal-content">
                            ...
                            <form name="form" onSubmit={this.savePermit}>
                                    ...
                                    <div>
                                        <div className="modal-body">
                                            ...
                                            }
                                            {
                                                ...
                                                <div className="form-group">
                                                    <label>
                                                        {this.state.driverlabel} </label>
                                                    <select
                                                        data-val="true"
                                                        value={this.state.driverid}
                                                        onChange={(e) => this.driverChange(e)}
                                                        className="edit-form " required>
                                                        <option value="">{this.state.driverlabel}</option>
                                            ...
                                            {this.state.permittypecode === "FO-FR10" &&
                                                <div className="form-group" style={{ marginTop: '10px' }}>
                                                    <table><tr>
                                            ...
                                            {this.state.permittypecode === "FO-FR03" &&
                                                <div className="form-group" style={{ marginTop: '10px' }}>
                                                    <table><tr>
                                                    ...
                                                    <label style={{ width: '99%' }} htmlFor="documents">{this.props.t('Upload_Documents')}</label>&nbsp;
                                        {
                                            ...
                                            <div>
                                                <div className="form-group" style={{ textAlign: 'center', marginTop: '30px' }}>
                                                    <label>Pay For Permit - (
                                                        {
                                                            this.state.isthisreligionsschool === true && <span>
                                                                0</span>
                                                        }
                                                        {
                                                            this.state.isthisreligionsschool === false && <span>
                                ...
                                }
                                {
                                    ...
                                    < div className="modal-body">
                                        <div className="form-group" style={{ textAlign: 'center', marginTop: '30px' }}>
                                        <label>Pay For Permit - ({this.state.price} AED)</label>
                                        </div>
