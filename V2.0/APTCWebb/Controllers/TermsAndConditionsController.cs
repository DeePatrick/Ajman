...
namespace APTCWebb.Controllers
{
    ...
    public class TermsAndConditionsController : ApiController
    {
        ...
        public async Task<IActionResult> RegisterAboutUs(TermsAndConditions model)
        {
            try
            {
        ...
        [HttpPost]
        [ResponseType(typeof(MessageModel))]
        public async Task<IActionResult> UpdateTermsAndConditions([FromUri]string id, TermsAndConditions model)
        {
            try
            {
                if (!ModelState.IsValid)
