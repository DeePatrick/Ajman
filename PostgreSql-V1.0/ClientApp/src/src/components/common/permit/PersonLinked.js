...
class PersonLinked extends Component {
    displayName = PersonLinked.name;
    ...
    addOwnershipRoles = (e) => {
        ...
        fetch(config.webApiUrl() + "aptc_individual_emiratesid/" + localStorage.getItem('selectedLanguageCode') + "/" + e.target.value)
            .then(response => response.json())
            .then(data => {
        ...
        if (this.state.companyownerslist[0]['ownerroleid'] === "") {
            alert('please enter correct emirates id');
