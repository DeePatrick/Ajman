...
import PermitPayment from '../Permit/PermitPayment';
...
export class RequestPermit extends Component {
    ...
    bindEmployeebyrole(roleid) {
        fetch(config.webApiUrl() + "aptc_individual_getAllActiveDrivers/" + localStorage.getItem('selectedLanguageCode') + "/" + localStorage.getItem('individualid') + "/" + roleid)
            .then(response => response.json())
            .then(data => {
    ...
    driverChange = (e) => {
        ...
        if (this.state.driverlist.length > 0) {
            for (var i = 0; i < this.state.driverlist.length; i++) {
                if (this.state.driverlist[i].individualid.toString() === e.target.value) {
                                                    ...
                                                    <label style={{ width: '99%' }} htmlFor="documents">{this.props.t('Upload_Documents')}</label>&nbsp;
                                        {
                                            ...
                                            <div>
                                                <div className="form-group" style={{ textAlign: 'center', marginTop: '30px' }}>
                                                    <label>{this.props.t('Pay_For_Permit')} - (
                                                        {
